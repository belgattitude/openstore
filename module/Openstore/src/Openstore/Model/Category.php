...
class Category extends AbstractModel implements BrowsableInterface
{
    ...
    public function getAncestors($category, $language)
    {
        ...
        $select->from(['pc1' => 'product_category'], [])
                ->join(
                ...
                ->join(
                    ['pc18' => 'product_category_translation'],
        ...
        $select->where(['pc1.reference' => $category]);
        $select->where(['pc2.lvl > 0']);
        $select->order(['pc2.lvl' => $select::ORDER_ASCENDING]);
    ...
    public function getParent($category, $language)
    {
        ...
        $select->where(['pc.reference' => $category]);
